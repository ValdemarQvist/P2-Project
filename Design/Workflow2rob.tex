...
%Primary nodes:
\node (start) [startstop] {Rotor arrives V visual done};
\node (dec1) [decision, below of=start, yshift=-1.5cm] {Is the balancing machine empty?};
\node (pro1) [process, below of=dec1, yshift=-2cm] {Place rotor in the balancing machine};
\node (stop) [io, below of=pro1, yshift=-0.5cm] {Balancing done};
...
\node (gui4) [Guidebox, left of=stop, xshift=-1.25cm] {};
\node (gui5) [Guidebox, below of=gui4, yshift=1.9cm] {};
\node (gui6) [Guidebox, left of=gui4, xshift=1.9cm] {};
\draw [arrow] (gui6) -- (stop);

\draw [arrow] (gui2) -- (gui5);

%Caption:
\node (cap1) [Caption, below of=stop, yshift=-2cm] {This flowchart shows the process performed by the first UR5 in the work-cell.};
...
%Primary nodes:
\node (start) [startstop] {Balancing done};
\node (dec1) [decision, below of=start, yshift=-1.5cm] {Did the rotor fail balancing?};
\node (pro1) [process, below of=dec1, yshift=-2cm] {Move rotor to visual inspection};
\node (dec2) [decision, below of=pro1, yshift=-2cm] {Did the rotor pass visual inspection?};
\node (pro2) [process, below of=dec2, yshift=-2cm] {Move rotor to queue table};
\node (stop) [io, below of=pro2, yshift=-0.5cm] {Visual done};
...
\node (gui4) [Guidebox, left of=dec2, xshift=-1.25cm] {};
\node (gui5) [Guidebox, above of=gui4, yshift=-1.9cm] {};
\node (gui6) [Guidebox, left of=gui4, xshift=1.9cm] {};
\draw [arrow] (dec2) -- node[anchor=south] {no} (gui6);

\node (gui7) [Guidebox, left of=stop, xshift=-1.25cm] {};
\node (gui8) [Guidebox, below of=gui7, yshift=1.9cm] {};
\node (gui9) [Guidebox, left of=gui7, xshift=1.9cm] {};
\draw [arrow] (gui9) -- (stop);

\draw [arrow] (gui2) -- (gui8);
